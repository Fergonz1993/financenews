...
1. Keep CI merge-gates deterministic (lint/type/unit/integration/smoke/ratchet).
2. Raise critical-path backend coverage (`api/main.py`, ingestion services, repositories).
3. Enforce typed ingestion boundaries and strict connector validation.
4. Lock API contracts (`/api/articles`, `/api/analytics`, `/api/sources`, `/api/ingest/status`).
5. Harden admin mutation security (auth/rbac/rate-limit/audit trail).
6. Preserve idempotency + replay safety in ingestion/state transitions.
7. Maintain connector feature flags + runtime kill switches.
8. Keep staging gate + backup/restore drills green.
9. Improve ranking/dedup quality with measurable duplicate-rate reduction.
