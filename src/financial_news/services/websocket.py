...
from enhanced_news_summarizer import Article, Config, EnhancedNewsSummarizer
...
class RealTimeStreamManager:
    ...
    def _setup_stream_configs(self) -> dict[str, StreamConfig]:
        ...
        return {
            ...
            "newsfilter": StreamConfig(
                ...
                subscribe_message=(
                    {"type": "subscribe", "channels": ["financial", "earnings", "fed"]}
                    if newsfilter_key
                    else None
                ),
    ...
    def _create_websocket_handler(self, stream_name: str, stream_config: StreamConfig):
        ...
        def on_close(ws, close_status_code, close_msg):
            ...
            self.stats["connections_active"] = max(
                0, self.stats["connections_active"] - 1
            )
            ...
            if stream_config.enabled:
                threading.Timer(
                    ...
                    lambda: self._reconnect_stream(stream_name),
                ).start()

        return on_open, on_message, on_error, on_close

    def _parse_message(self, stream_name: str, data: dict) -> StreamEvent | None:
    ...
    def _parse_eodhd_message(self, stream_name: str, data: dict) -> StreamEvent | None:
    ...
    def _parse_binance_message(self, data: dict) -> StreamEvent | None:
    ...
    def _parse_news_message(self, data: dict) -> StreamEvent | None:
    ...
    def get_status_table(self) -> Table:
        ...
        for stream_name, config in self.streams.items():
            ...
            status = (
                "🟢 Connected"
                if self.connection_status.get(stream_name)
                else "🔴 Disconnected"
            )
            ...
            table.add_row(
                ...
                str(
                    self.stats["last_event_time"].strftime("%H:%M:%S")
