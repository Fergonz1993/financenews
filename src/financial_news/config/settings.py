...
class Settings:
    ...
    def from_env(cls) -> "Settings":
        ...
        settings.database.password = os.getenv(
        ...
        settings.api.debug = (
            os.getenv("API_DEBUG", str(settings.api.debug)).lower() == "true"
        )

        # News sources
        settings.news_sources.alpha_vantage_api_key = os.getenv(
        ...
        # ML settings
        settings.ml.model_cache_dir = os.getenv(
            "MODEL_CACHE_DIR", settings.ml.model_cache_dir
        )
        settings.ml.sentiment_model = os.getenv(
            "SENTIMENT_MODEL", settings.ml.sentiment_model
        )
        settings.ml.summarization_model = os.getenv(
            "SUMMARIZATION_MODEL", settings.ml.summarization_model
        )

        # WebSocket settings
        settings.websocket.host = os.getenv("WS_HOST", settings.websocket.host)
        settings.websocket.port = int(
            os.getenv("WS_PORT", str(settings.websocket.port))
        )
