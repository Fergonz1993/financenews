...
class EventBuffer:
    ...
    def get_events_by_ticker(self, ticker: str, limit: int = 50) -> List[StreamingNewsEvent]:
    ...
    def get_priority_events(self, priority: EventPriority, limit: int = 20) -> List[StreamingNewsEvent]:
...
class StreamingNewsClassifier:
    ...
    def __init__(self):
        ...
        self.critical_patterns = {
            ...
            'regulatory': ['fda approval', 'regulatory approval', 'investigation', 'lawsuit'],
            'economic': ['fed', 'interest rate', 'inflation', 'gdp', 'unemployment']
        }
        
        self.sentiment_patterns = {
            'very_positive': ['soars', 'surges', 'beats estimates', 'record high', 'breakthrough'],
            'positive': ['rises', 'gains', 'up', 'positive', 'strong'],
            'negative': ['falls', 'drops', 'misses', 'disappoints', 'weak'],
            'very_negative': ['plummets', 'crashes', 'collapses', 'investigation', 'bankruptcy']
        }
    
    def classify_event(self, title: str, content: str, source: str) -> Tuple[NewsEventType, EventPriority]:
    ...
    def extract_tickers(self, title: str, content: str) -> List[str]:
        ...
        false_positives = {'THE', 'AND', 'FOR', 'ARE', 'BUT', 'NOT', 'YOU', 'ALL', 'CAN', 'HER', 'WAS', 'ONE', 'OUR', 'HAD', 'BY', 'BUT', 'WHO', 'FROM', 'THIS', 'NEW', 'NOW', 'OLD', 'SEE', 'TWO', 'WAY', 'ITS', 'DID', 'GET', 'MAY', 'HIM', 'HIS', 'HAS', 'OFF', 'SET', 'TOP', 'PUT', 'END', 'WHY', 'TRY', 'GOT', 'RUN', 'HOT', 'CUT', 'LET', 'YES', 'YET', 'SHE', 'USE', 'OWN', 'SAY', 'SHE', 'HOW', 'MAN', 'BOY', 'BIG', 'BAD', 'FEW', 'DAY', 'LOT', 'WAR', 'FAR', 'AGO', 'WON', 'ANY', 'SUN', 'SON', 'ART', 'ACT', 'AGE', 'EYE', 'ARM', 'EAR', 'EGG', 'AID', 'ICE', 'OIL', 'AIR', 'BAG', 'BAR', 'BAT', 'BED', 'BEE', 'BIT', 'BOX', 'BUS', 'BUG', 'CAR', 'CAT', 'COW', 'CUP', 'DOG', 'ELF', 'FAN', 'FIG', 'FOX', 'GAS', 'GUN', 'HAT', 'HEN', 'JAR', 'JOB', 'KEY', 'KID', 'LAW', 'LEG', 'LID', 'LOG', 'MOM', 'NET', 'NUT', 'PAN', 'PEN', 'PET', 'PIE', 'PIG', 'POT', 'RAT', 'RED', 'SIR', 'SIX', 'SKY', 'TAX', 'TEN', 'TIE', 'TIP', 'TOY', 'VAN', 'WEB', 'WIN', 'ZIP'}
    ...
    def _determine_priority(self, text: str, event_type: NewsEventType, source: str) -> EventPriority:
        ...
        high_priority_sources = ['reuters', 'bloomberg', 'cnbc', 'sec', 'federal reserve']
        if any(source_name in source.lower() for source_name in high_priority_sources):
            if event_type in [NewsEventType.EARNINGS, NewsEventType.ACQUISITION, NewsEventType.REGULATORY]:
                return EventPriority.HIGH
        
        # Medium priority for sector updates
        if event_type in [NewsEventType.ANALYST_UPGRADE, NewsEventType.ANALYST_DOWNGRADE]:
...
class RealTimeNewsStream:
    ...
    async def _send_initial_data(self, websocket):
        ...
        recent_events = self.event_buffer.get_priority_events(EventPriority.CRITICAL, limit=10)
        recent_events.extend(self.event_buffer.get_priority_events(EventPriority.HIGH, limit=10))
            ...
            while self.is_running:
                try:
                    async with self.throttler:
                        await self._fetch_and_process_news(session, source_url, source_name)
    ...
    async def _fetch_and_process_news(self, session: aiohttp.ClientSession, 
    ...
    async def _create_and_process_event(self, article: Dict, source_name: str):
        ...
        # Classify event
        event_type, priority = self.classifier.classify_event(title, content, source_name)
    ...
    async def get_live_dashboard_data(self) -> Dict:
        ...
        critical_events = self.event_buffer.get_priority_events(EventPriority.CRITICAL, limit=5)
        high_events = self.event_buffer.get_priority_events(EventPriority.HIGH, limit=10)
        ...
        if hasattr(self, '_last_metrics_time'):
            ...
            if time_diff > 0:
                events_diff = self.metrics['events_processed'] - getattr(self, '_last_events_count', 0)
        ...
        def generate_dashboard():
            ...
            try:
                ...
                if loop.is_running():
                    ...
                    task = asyncio.create_task(self.stream_manager.get_live_dashboard_data())
                    data = {}  # Fallback to empty data for now
                else:
                    data = asyncio.run(self.stream_manager.get_live_dashboard_data())
            except Exception:
                data = {'metrics': {}, 'critical_events': [], 'high_priority_events': []}
            ...
            metrics_table.add_row("Events Processed", str(metrics.get('events_processed', 0)))
            metrics_table.add_row("Events/Second", str(metrics.get('events_per_second', 0)))
            metrics_table.add_row("Active Connections", str(metrics.get('connection_count', 0)))
...
async def start_realtime_analysis(config: Dict):
    ...
    news_sources = config.get('news_sources', [
        {'url': 'https://newsapi.org/v2/everything?q=stocks&apiKey=YOUR_KEY', 'name': 'NewsAPI'},
        {'url': 'https://api.marketaux.com/v1/news/all?api_token=YOUR_TOKEN', 'name': 'MarketAux'}
    ])
