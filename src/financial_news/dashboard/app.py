...
try:
    from enhanced_news_summarizer import (
        run_enhanced_summarizer,
    )
except ImportError:
    st.error(
        "Enhanced news summarizer module not found. Please ensure enhanced_news_summarizer.py is in the same directory."
    )
    st.stop()
...
def display_analytics_dashboard(analytics):
    ...
    with col1:
        st.metric(label="📰 Total Articles", value=analytics.get("total_articles", 0))
    ...
    with col4:
        ...
        st.metric(label="⚡ Avg Processing Time", value=f"{avg_time:.1f}s")
    ...
    with col1:
        ...
        sentiment_dist = analytics.get("sentiment_distribution", {}).get(
            "distribution", {}
        )
        if sentiment_dist:
...
def display_articles(articles):
    ...
    sorted_articles = sorted(
        articles, key=lambda x: x.get("market_impact_score", 0), reverse=True
    )

    for _i, article in enumerate(sorted_articles):
        ...
        with st.container():
            st.markdown(
                f'<div class="article-card {impact_class}">', unsafe_allow_html=True
            )
            ...
            with col1:
                title = article.get("summarized_headline") or article.get(
                    "title", "Untitled"
                )
                url = article.get("url", "#")
            ...
            with col3:
                st.markdown(
                    f'<span class="{sentiment_class}">{sentiment_emoji} {sentiment.title()}</span>',
                    unsafe_allow_html=True,
                )
...
def main():
    ...
    # Header
    st.markdown(
        '<h1 class="main-header">📈 Financial News Summarizer Dashboard</h1>',
        unsafe_allow_html=True,
    )
    ...
    tab1, tab2, tab3 = st.tabs(
        ["🚀 Run Analysis", "📊 Analytics Dashboard", "📰 Articles"]
    )
            ...
            with col1:
                if st.button(
                    ...
                    if not openai_key or openai_key == "your_openai_api_key_here":
                        st.error(
                            "❌ OpenAI API key is required. Please configure it in your .env file."
                        )
                    else:
                        # Run analysis
                        ...
                        if success:
                            st.success(
                                "✅ Analysis completed! Check the other tabs for results."
                            )
                            st.experimental_rerun()

            with col2:
                st.info(
                    "💡 Analysis will fetch news articles, analyze them with AI, and generate comprehensive reports."
                )
    ...
    with tab3:
        ...
        if articles:
            ...
            with col3:
                st.selectbox(
                    "Sort by:", ["Market Impact", "Sentiment Score", "Published Date"]
                )
            ...
            if impact_filter != "All":
                if impact_filter.startswith("High"):
                    filtered_articles = [
                        a
                        for a in filtered_articles
                        if a.get("market_impact_score", 0) > 0.7
                    ]
                elif impact_filter.startswith("Medium"):
                    filtered_articles = [
                        a
                        for a in filtered_articles
                        if 0.4 <= a.get("market_impact_score", 0) <= 0.7
                    ]
                elif impact_filter.startswith("Low"):
                    filtered_articles = [
                        a
                        for a in filtered_articles
                        if a.get("market_impact_score", 0) < 0.4
                    ]

            st.write(
                f"**Showing {len(filtered_articles)} of {len(articles)} articles**"
            )
